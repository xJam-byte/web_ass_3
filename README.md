...
5) Enhanced JS (25)  
   - Auth: локально без бэкенда.  
     - Ключи: users (массив), currentUser (объект).  
     - После логина — редирект на profile.html с выводом имени/почты.  
   - Validation: email, пароль (≥8, верхн/нижн/цифра), телефон +7-формат, required-поля.  
...
- [ ] Логин/регистрация/профиль работают  
...
- Auth — демонстрационный (без сервера)  
